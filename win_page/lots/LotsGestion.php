<?php
	class LotsGestion {
		public function getRandomFreeLot($bdd, $session) {
			...
			$req = $bdd->prepare('SELECT * FROM lots_list WHERE use_left_nb>0 AND key_type=0 ORDER BY RAND()');
			$req->execute(array());
		...
		public function getRandomSudokuLot($bdd, $session) {
			...
			$req = $bdd->prepare('SELECT * FROM lots_list WHERE use_left_nb>0 AND key_type=1 ORDER BY RAND()');
			$req->execute(array());
